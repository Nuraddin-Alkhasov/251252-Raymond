...
namespace HMI.Resources.UserControls
{
    public partial class ArticleRecipe : UserControl
    {
        ...
        {
            IRecipeClass Class = ApplicationService.GetService<IRecipeService>().GetRecipeClass("LD");
            ...
            Class = ApplicationService.GetService<IRecipeService>().GetRecipeClass("BT");
            VariableNames = Class.GetVariableNames();
            foreach (string vn in VariableNames)
            {
                Class.SetValue(vn, 0);
            }
            Class = ApplicationService.GetService<IRecipeService>().GetRecipeClass("PO");
            VariableNames = Class.GetVariableNames();
            foreach (string vn in VariableNames)
            {
                Class.SetValue(vn, 0);
