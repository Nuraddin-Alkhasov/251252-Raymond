...
namespace HMI.DialogRegion.MO.Views
{
    ...
    public partial class M2_WZ_EF
    {
        ...
        private void View_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            VPurge = VS.GetVariable("CPU4.PLC.Blocks.04 Tray handling.05 WZ.02 Exhaust.DB WZ Exhaust HMI.Actual.Purge.On");
            VPurge.Change += VPurge_Change;

            VPS1 = VS.GetVariable("CPU4.PLC.Blocks.04 Tray handling.05 WZ.02 Exhaust.DB WZ Exhaust HMI.Actual.Purge.Pressure switch 1");
            VPS1.Change += VPS1_Change;

            VPS2 = VS.GetVariable("CPU4.PLC.Blocks.04 Tray handling.05 WZ.02 Exhaust.DB WZ Exhaust HMI.Actual.Purge.Pressure switch 2");
        ...
        private void V_Off_Click(object sender, RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@MachineOverview.Text53", "@Buttons.Text64", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU4.PLC.Blocks.04 Tray handling.05 WZ.02 Exhaust.DB WZ Exhaust HMI.PC.Drive.from.Restart", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU4.PLC.Blocks.04 Tray handling.05 WZ.02 Exhaust.DB WZ Exhaust HMI.PC.Drive.from.Restart", false);
