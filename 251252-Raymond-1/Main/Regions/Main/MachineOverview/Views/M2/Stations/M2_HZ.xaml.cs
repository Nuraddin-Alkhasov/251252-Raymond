...
namespace HMI.MainRegion.MO.Views
{
    ...
    public partial class M2_HZ
    {
        ...
        {
            InitializeComponent();
            Clocked = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.00 Main.DB HZ HMI.Parameter.Stack[0].Place";
        ...
        private void IVClocked_ValueChanged(object sender, VariableEventArgs e)
        {
            if (e.Quality.Data == DataQuality.Good)
            {
                if (this.IsVisible)
                {
                    byte HZ = (byte)(((byte)ApplicationService.GetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.00 Main.DB HZ HMI.Parameter.Stack[0].Id")) == 1 ? 13 : 14);
                    byte Place = (byte)ApplicationService.GetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.00 Main.DB HZ HMI.Parameter.Stack[0].Place");
        ...
        {
            byte HZ = (byte)ApplicationService.GetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.00 Main.DB HZ HMI.Parameter.Stack[0].Id");
            byte Place = (byte)ApplicationService.GetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.00 Main.DB HZ HMI.Parameter.Stack[0].Place");
        ...
        private HZTray_L GetHZTray_L(int i, bool head, byte hz)
        {
            return new HZTray_L()
            {
                IsTray = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Tray.Available", 
                IsMaterial = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Charge.Available",
                SetLayer = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Charge.Layer[1].Set",
                ActualLayer = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Charge.Layer[1].Actual",
                IsDischarge = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Tray.Functions.Discharge",
                IsWatch = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Tray.Functions.Watch",
                Charge = "CPU4.PLC.Blocks.04 Tray handling.06 HZ.0" + hz + ".00 Main.DB HZ " + hz + " PD.Place[" + i + "].Header.Charge",
                Station = hz == 1 ? 13 : 14,
                Place = i,
                Head = head,
                Type = "Tray",
