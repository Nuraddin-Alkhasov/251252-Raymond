...
namespace HMI.DialogRegion.MO.Views
{
    ...
    [ExportView("M2_HZ1_EF")]
    public partial class M2_HZ1_EF
    {
        public M2_HZ1_EF()
        {
            InitializeComponent();
        ...
        private void View_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            VPS1 = VS.GetVariable("CPU4.PLC.Blocks.04 Tray handling.06 HZ.01.02 Exhaust.DB HZ 1 Exhaust HMI.Actual.Purge.Pressure switch 1");
            VPS1.Change += VPS1_Change;

            VPS2 = VS.GetVariable("CPU4.PLC.Blocks.04 Tray handling.06 HZ.01.02 Exhaust.DB HZ 1 Exhaust HMI.Actual.Purge.Pressure switch 2");
        ...
        private void V_Off_Click(object sender, RoutedEventArgs e)
        {
            if (MessageBoxView.Show("@MachineOverview.Text53", "@Buttons.Text64", MessageBoxButton.YesNo, MessageBoxResult.No, MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.01.02 Exhaust.DB HZ 1 Exhaust HMI.PC.Drive.from.Restart", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU4.PLC.Blocks.04 Tray handling.06 HZ.01.02 Exhaust.DB HZ 1 Exhaust HMI.PC.Drive.from.Restart", false);
