...
namespace HMI.DialogRegion.MO.Views
{
    ...
    [ExportView("M2_Coating_VC")]
    public partial class M2_Coating_VC
    {
        public M2_Coating_VC()
        {
            InitializeComponent();
        }
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (MessageBoxView.Show("@MachineOverview.Text53", "@MachineOverview.Text45", MessageBoxButton.YesNo, icon: MessageBoxIcon.Question) == MessageBoxResult.Yes)
            {
                ...
                Task taskA = Task.Run(() =>
                {
                    ApplicationService.SetVariableValue("CPU3.PLC.Blocks.03 Basket coating.02 DT.00 Main.DB DT HMI.PC.Data.from.Viscosity.Checked", true);
                });
                taskA.ContinueWith(async x =>
                {
                    await Task.Delay(1000);
                    ApplicationService.SetVariableValue("CPU3.PLC.Blocks.03 Basket coating.02 DT.00 Main.DB DT HMI.PC.Data.from.Viscosity.Checked", false);
